...
[explore] desktop footer (`Footer.tsx`): `hidden md:block`, `bg-footer-bg border-t border-white-10 pt-xxxl pb-xl px-xl`, max-w `1100px` centered. Top row: brand (logo 130×30) + 4 link columns (CATEGORIES, COMPANY, SUPPORT, FOLLOW US) with `gap-xl`. Column headings: `text-text-dim text-xxs font-semibold tracking-widest uppercase mb-m`. Column links: `text-text-body text-sm hover:text-text-title gap-s`. Social links have inline SVG icons (18×18) + label. Policies row: `border-t border-white-10 pt-m mb-s`. Heading: `text-text-dim text-xxs font-semibold tracking-widest uppercase mb-s`. Links: `flex flex-wrap gap-x-l gap-y-xs`, each `text-text-dim text-xs hover:text-text-small inline-flex items-center` + ExternalLinkIcon (12×12, diagonal-arrow style from WhatIsWsup, `stroke="currentColor" strokeOpacity="0.7" strokeWidth="1.3"`). 13 policies: Community, Law Enforcement Request, Cookies, DMCA & IP, NCII, Safety & Crisis, Terms of Use, Privacy Policy, Refund Policy, Appeals Policy, Cancellation Policy, Contact Us, Complaints Policy. Copyright: `pt-m flex items-center justify-between`, year text `text-text-xxsmall text-xs`, privacy choices button `gap-xs` (token, NOT `gap-2`).

[explore] mobile footer (`FooterMobile.tsx`): `md:hidden bg-footer-bg`. Top row: logo (104×24) + social icons (Discord, Instagram, Reddit) — same SVGs as desktop `Footer.tsx`. Social icon color: `text-text-subtitle` (80% white). Accordion rows: `h-[84px] border-t border-white-10 flex items-center justify-between px-m`. Row label: `text-text-subtitle font-semibold text-base`. One accordion section: Categories. Four nav rows: About Us, Download App (ChevronRight), AI Companion, Blog (ExternalLinkIcon). Default open: none (`useState<AccordionKey|null>(null)`). Expanded content: `border-t border-white-10 flex flex-col px-l py-s`. Sub-links: `py-s text-text-xsmall text-base hover:text-text-body`. Policies section: flat wrapped list (NOT accordion) — matches desktop layout. Container: `border-t border-white-10 px-l pt-m pb-m`. Heading: `text-text-dim text-xxs font-semibold tracking-widest uppercase mb-m`. Links grid: `flex flex-wrap gap-x-m gap-y-s`. Each link: `text-text-dim text-xs hover:text-text-small inline-flex items-center` + ExternalLinkIcon (size 14). 13 policies: Community, Law Enforcement Request, Cookies, DMCA & IP, NCII, Safety & Crisis, Terms of Use, Privacy Policy, Refund Policy, Appeals Policy, Cancellation Policy, Contact Us, Complaints Policy. ExternalLinkIcon: diagonal-arrow style from WhatIsWsup — viewBox `0 0 14 14`, two paths (L-bracket + arrow), `stroke="currentColor" strokeOpacity="0.7" strokeWidth="1.3"`. Accepts `size` prop (default 20 for NavRow, 14 for policy links). Copyright padding: `pb-[calc(80px+env(safe-area-inset-bottom,0px))]` — clears BottomNav height + iPhone safe area.
