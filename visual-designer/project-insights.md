...
| **StreakClaimPopup** | ~~auto-opens on /explore load~~ HIDDEN during Wind-Down phase (S35 close — see Wind-Down section). Mount removed from `explore/page.tsx`; component file retained for post-wind-down restoration | BottomSheet (mobile) / CenterPopup (desktop) | 70 | Pre-wind-down behavior: independent of onboarding flow; auto-opened on every /explore visit. Chat-context triggers (new-user post-login routing via `chatState === 'claim-credits-popup'`) still work — only the /explore auto-open was removed |
