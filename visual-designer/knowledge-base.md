...
Last updated: 2026-05-18 (S35 audit — 3 new entries: Wind-Down Notice global system architecture + CloseButton position WSUP majority convention + dynamic-layout-offset CSS-var pattern for layout chrome)

### Wind-Down Notice — single-exposure forced-acknowledgment popup + details popup (REVISED S35 close)
...
- **Surface 1 — first-exposure popup (BOTH viewports, parallel mount):** `<BottomSheet>` + `<CenterPopup>` rendered as siblings per S34 responsive-popup pattern. 64px alert hero icon at top-center (sibling-inheritance from `StatusResultIcon` chrome — `size-[64px] rounded-full bg-status-X/[0.15] border border-status-X/[0.30]` + colored glyph), title + body + "Other apps to try" section + primary acknowledgment CTA + tertiary "Read more" link. **Forced single-exposure:** no close X, no scrim-dismiss, no Esc-dismiss — `onClose={() => {}}` passed to both BottomSheet and CenterPopup primitives. The "Okay, I understand" primary CTA is the ONLY exit path. Gated by localStorage permanent flag (e.g. `wsup:winddown-popup-seen`). Once acknowledged, never shows again on that device.
- **Surface 2 — details popup (BOTH viewports, parallel mount):** same parallel-mount pattern. Long-form scrollable info (reasoning + 3-phase timeline + "Other apps to try" + personal note + email contact lines). NO action CTAs — refund + download requests funnel to email (placeholders: refund@wsup.ai / data@wsup.ai) to minimize one-tap query volume. Email contact flows naturally after content (not sticky). Reachable from "Read the full update" on the first-exposure popup.

**Why forced acknowledgment with NO persistent reminder chrome:** for one-time critical-info delivery, the popup's force-acknowledgment is the contract — once the user explicitly taps the CTA, they've acknowledged. Persistent reminder chrome (e.g. a top strip) adds visual cost without proportional value once the user has acknowledged. This is the right trade-off when the message is delivered once and persists in user memory (or the news ecosystem). For time-bounded warnings that need periodic re-surfacing, use the WSUP status-banner pattern (LowCreditsBanner) instead.
...
**Codified by:** WindDownNotice + WindDownPopup + WindDownDetailsPopup (S35). **Pre-flight check before building any new system-wide announcement:** ask *"is this a one-time critical-info delivery (user acknowledges once and we're done), or a persistent state that needs periodic re-surfacing?"* One-time → this 2-surface forced-acknowledgment pattern. Persistent → status-banner pattern (LowCreditsBanner-style). **REVISION NOTE:** S35 morning briefly codified a 3-surface popup-then-strip degradation pattern; designer simplified to single-exposure model at S35 close (Gate 6.5 outcome path #3 — invalidated the just-codified rule). The current entry reflects the codified state at session close.
...
### Dynamic layout offset via CSS var — load-bearing chrome that shows/hides should not require prop drilling (DOCUMENTED, INACTIVE — strip removed S35 close)
When a chrome element (top strip, banner row, announcement bar) toggles visibility AND affects the layout of fixed-position siblings (Header, Sidebar) or page contents (main `pt-*`), use a CSS variable on `document.documentElement` rather than React context or prop drilling. The chrome's component publishes its actual rendered height via `useLayoutEffect` + `ResizeObserver`; downstream consumers (Header, Sidebar, pages) read `var(--chrome-height, 0px)` via inline `style={{ top: '...' }}` or a Tailwind spacing token computed via `calc()`. **Why:** layout chrome consumers are often outside React's render tree (each page owns its own padding; Header and Sidebar mount independently per page). A CSS var reaches all of them with zero coupling and updates atomically on resize. A React context would require wrapping every page; prop drilling would touch 5+ files for every layout-affecting chrome change. **Status (S35 close): DOCUMENTED BUT INACTIVE** — the wind-down strip that originally drove this pattern was removed by designer call at S35 close. The pattern is preserved here for future reference; the `header` Tailwind spacing token in `tailwind.config.ts` was simplified from `calc(60px + var(...))` back to `60px`. Pages still use `pt-header` / `mt-header` for semantic clarity but the dynamic offset is no longer in use. **Pre-flight check before adding a new top-of-page chrome:** does it toggle visibility? Does its height vary by viewport? If both yes → CSS var pattern. If neither → static offset is fine.
