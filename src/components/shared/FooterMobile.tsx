...
const ChevronRight = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
    <path d="M8 5l5 5-5 5" stroke="rgba(255,255,255,0.5)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
...
// ── Social icons (same SVGs as Footer.tsx) ────────────────────────────────────
...
function AccordionRow({
  ...
  return (
    <button
      onClick={onToggle}
      className="w-full h-[84px] flex items-center justify-between px-m border-t border-white-10 hover:bg-white-05 transition-colors"
    >
...
function NavRow({ label, external = false }: { label: string; external?: boolean }) {
  return (
    <Link
      href="#"
...
export default function FooterMobile() {
  ...
  return (
    <footer className="md:hidden bg-footer-bg">
      {/* Logo + Social */}
      <div className="flex items-center justify-between px-l py-l border-t border-white-10">
        ...
        <div className="flex items-center gap-l text-text-subtitle">
          <button className="hover:text-text-title transition-colors"><DiscordIcon /></button>
          <button className="hover:text-text-title transition-colors"><InstagramIcon /></button>
          <button className="hover:text-text-title transition-colors"><RedditIcon /></button>
        </div>
      </div>
      ...
      <div>
        {/* Categories */}
        <AccordionRow label="Categories" isOpen={open === 'categories'} onToggle={() => toggle('categories')} />
        {open === 'categories' && (
          <div className="border-t border-white-10 flex flex-col px-l py-s">
        ...
        <NavRow label="About Us" />
        <NavRow label="Download App" />
        <NavRow label="AI Companion" external />
