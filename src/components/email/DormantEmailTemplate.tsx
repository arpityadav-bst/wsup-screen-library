const INACTIVE_CHARS = [
  { name: 'Mika', chats: '18.1K chats', deadline: 'Removed in 52 days', img: '/chars/char10.webp' },
  { name: 'Harlo', chats: '21.8K chats', deadline: 'Removed in 48 days', img: '/chars/char11.webp' },
]

const POLICY_CHARS = [
  { name: 'Joo Jaekyung', chats: '48.4K chats', deadline: 'Removed in 38 days', img: '/chars/char12.webp' },
  { name: 'Roblox Story', chats: '90.4K chats', deadline: 'Removed in 35 days', img: '/chars/char13.webp' },
  { name: 'Arianda Grande', chats: '25.1K chats', deadline: 'Removed in 40 days', img: '/chars/char14.webp' },
]
  ...
  return (
    <div style={{ width: '100%', maxWidth: 600, background: '#171717', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.04)', overflow: 'hidden', fontFamily: "'Rubik', sans-serif" }}>
      ...
      <div style={{ padding: '28px 24px 0' }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', margin: '0 0 12px' }}>Hi Creator,</h2>
        <p style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(255,255,255,0.7)', margin: 0 }}>
          5 of your characters on wsup.ai need your attention. They have been marked dormant and will be permanently removed unless you take action.
        </p>
      </div>

      {/* Summary stats — dot + number + label, matching app filter pills */}
      <div style={{ display: 'flex', gap: 10, padding: '20px 24px' }}>
        <StatPill dot="rgba(255,255,255,0.4)" value="2" label="Inactive" />
        <StatPill dot="#ffc32a" value="3" label="Policy Review" />
      </div>
      ...
      <div style={{ padding: '24px 24px 0' }}>
        <a href="#" style={{ display: 'block', width: '100%', height: 48, background: '#4a3ec6', color: '#ffffff', border: 'none', borderRadius: 9999, fontSize: 16, fontWeight: 600, textAlign: 'center', lineHeight: '48px', textDecoration: 'none', boxSizing: 'border-box' }}>
      ...
      <div style={{ padding: '28px 24px 0' }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {[
            ...
            'Edit and re-submit characters for content review',
            'Revive costs 20 credits per character (covers moderation review)',
          ].map((item) => (
...
function StatPill({ dot, value, label }: { dot: string; value: string; label: string }) {
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 9999 }}>
...
function SectionHeader({ label, count, dotColor }: { label: string; count: number; dotColor: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 0 4px' }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', background: 'rgba(255,255,255,0.08)', borderRadius: 100, padding: '1px 8px' }}>{count}</span>
    </div>
  )
}

function CharRow({ name, badge, badgeType, chats, deadline, img, isLast }: {
  name: string; badge: string; badgeType: 'inactive' | 'policy'
  chats: string; deadline: string; img: string; isLast: boolean
}) {
  const badgeStyle = badgeType === 'policy'
    ? { background: 'rgba(255,195,42,0.15)', color: '#ffc32a' }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '14px 0', borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.06)', gap: 12 }}>
      ...
      <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
        <div style={{ fontSize: 12, color: '#ffc32a', fontWeight: 500, whiteSpace: 'nowrap' as const }}>{deadline}</div>
