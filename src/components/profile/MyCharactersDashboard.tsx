...
import RemovedCharacterCard from './RemovedCharacterCard'
import type { RemovedCharacter } from './RemovedCharacterCard'
...
interface MyCharactersDashboardProps {
  ...
  dormantChars: DormantCharacter[]
  removedChars: RemovedCharacter[]
...
export default function MyCharactersDashboard({
  ...
  const pills: FilterPill[] = [
    ...
    { label: 'Needs Attention', count: dormantChars.length, dot: '#ffc32a' },
  ...
  return (
    <div className="px-m md:px-4xl py-m flex flex-col gap-xl">
      ...
      <div className="flex items-center gap-xs">
        ...
        {onStatesInfo && (
          <button
            ...
            className="shrink-0 w-xxl h-xxl flex items-center justify-center rounded-full bg-transparent border border-white-20 cursor-pointer hover:bg-white-05 hover:border-white-30 transition-colors text-text-dim"
      ...
      {showNeedsAttention && dormantChars.length > 0 && (
        <div className="flex flex-col gap-s">
          <div className="flex items-center gap-xs">
            <svg className="w-[18px] h-[18px] text-status-warning shrink-0" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-xs md:gap-s">
            {dormantChars.map((char) => (
              <DormantCharacterCard key={char.name} {...char} onBadgeClick={onStatesInfo} />
            ))}
          </div>
        </div>
      )}
      ...
      {showNeedsAttention && dormantChars.length > 0 && showActive && activeChars.length > 0 && (
      ...
      {showRemoved && removedChars.length > 0 && ((showActive && activeChars.length > 0) || (showNeedsAttention && dormantChars.length > 0)) && (
        <div className="h-px bg-white-10" />
      )}
        ...
        <div className="flex flex-col gap-s">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-xs md:gap-s">
            {removedChars.map((char) => (
              <RemovedCharacterCard key={char.name} {...char} />
            ))}
          </div>
        </div>
      )}
