  ...
  return (
    <Section id="Widgets" title="Widgets" onVisible={onSectionVisible}>
      ...
      <div className="min-w-[440px] flex-1">
        <SubLabel>Buy Credits Promo (BuyCreditsPromoCard)</SubLabel>
        <p className="text-text-xsmall text-xs mb-3">Shared promo card in <code className="text-accent-light">components/shared/BuyCreditsPromoCard.tsx</code>. Used by <code className="text-accent-light">CreditSidebar</code> AND <code className="text-accent-light">CreditServicePopup</code> (extracted Gate-3 style in S31 when the popup became the second consumer). Dark card with orange/yellow radial gradients + detailed bag illustration (<code className="text-accent-light">/credit-bags.png</code>) overflowing top-right. Primary &quot;Buy Credits&quot; button at 180×40. Default <code className="text-accent-light">onBuy</code> dispatches <code className="text-accent-light">wsup:open-buy-credits</code> event; pass an explicit handler when the consumer wants local control.</p>
        <div className="flex flex-col gap-3">
          <PreviewBox>
            <div className="w-[320px]"><BuyCreditsPromoCard onBuy={() => {}} /></div>
          </PreviewBox>
          <StateLabel>Default — imports the actual component so any future change auto-syncs (no inline duplication).</StateLabel>
        </div>
      </div>
