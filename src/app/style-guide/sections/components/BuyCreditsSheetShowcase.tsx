    ...
    <div className="w-[382px] bg-profile-sheet-bg border border-white-10 rounded-popup shadow-popup overflow-hidden" style={{ backgroundImage: SHELL_BG }}>
      ...
      <div className="flex flex-col items-center px-l pb-l text-center min-h-[380px]">
        ...
        <div className="flex flex-col gap-s items-center w-full">
          ...
          {isSuccess && <p className="text-xs text-text-small">Receipt emailed to you@example.com</p>}
          {!isSuccess && <span className="link text-xs">Contact support</span>}
        </div>
      </div>
    </div>
  )
...
export default function BuyCreditsSheetShowcase() {
  ...
  return (
    <div className="min-w-[440px] flex-1">
      ...
      <StateLabel>4 packs (2 shown). Each row carries its own &quot;Buy for {`{price}`}&quot; CTA — tapping advances directly to the Payment step.</StateLabel>
      ...
      <StateLabel>Shown after payment. Success: &quot;Credits added · +X · total Y&quot;. Failure: retry-able with support link.</StateLabel>

      <div className="mt-m flex flex-col gap-xxs">
        <TokenRow token="BuyCreditsSheet" desc="src/components/ui/BuyCreditsSheet.tsx — orchestrator with step state (one-time only as of S31)" />
        <TokenRow token="BuyCreditsPackagesStep" desc="src/components/ui/BuyCreditsPackagesStep.tsx — pack list, each row has its own Buy CTA" />
        <TokenRow token="CreditPackRow" desc="src/components/ui/CreditPackRow.tsx — pack name + credit count + per-rupee rate + Buy-for-{price} CTA. Featured variant gets gold gradient" />
        <TokenRow token="BuyCreditsResultStep" desc="src/components/ui/BuyCreditsResultStep.tsx — success/failure variant" />
        <TokenRow token="CreditsSummaryPill" desc="src/components/ui/CreditsSummaryPill.tsx — shared by Payment + Scan steps" />
        ...
        <TokenRow token="Flow" desc="packages → payment → scan (QR desktop / app mobile) → result" />
        <TokenRow token="contextNote prop" desc="Optional string. When provided, renders a text-xs text-text-body line under the &quot;Buy credits&quot; step header (used by chat-screen out-of-credits gate to explain why the sheet appeared)" />
        <TokenRow token="Monthly subscription" desc="Removed in S31 — going forward with one-time only. PackModeToggle, applyMonthlyBonus, ResultMode='subscription', selectable CreditPackRow variant all dropped. Reintroduce as non-breaking additions if monthly returns" />
      </div>
    </div>
  )
}
