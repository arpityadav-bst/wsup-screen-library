  ...
  return (
    <Section id="Watch Ad Gate" title="Watch Ad Gate" onVisible={onSectionVisible}>
      <div className="w-full">
        <SubLabel>Mobile + desktop. Hard send-gate that ships TWO distinct flows — each is its own dev-panel Flow option, each triggered by sending a message (not by selecting the flow). When &ldquo;Ad flow - Bottom sheet&rdquo; is active, the gate fires every Nth send as a modal popup — BottomSheet on mobile, CenterPopup on desktop (parallel-mount pattern matching BuyCreditsSheet; each primitive hides itself on the wrong viewport). When &ldquo;Ad flow - AI chat bubble&rdquo; is active, the gate fires every Nth send as an inline AI-bubble in the chat stream — same surface on both viewports (inherits the chat column&apos;s responsive width via <code className="text-accent-light">max-w-chat-bubble</code>). N batched at 9 / 16 / 24 / 32 per product config; demo uses 1 free send (2nd attempt fires — gate appears right after the 1st &ldquo;hi&rdquo;). The user&apos;s draft is held while the gate is up; tapping &ldquo;Watch ad&rdquo; releases the draft and sends. DummyAd post-tap: portrait asset on mobile, landscape asset on desktop.</SubLabel>
      </div>
      ...
      <div className="min-w-[440px] flex-1">
        ...
        <p className="text-text-xsmall text-xs mb-3">Placeholder ad surface — real Homescapes screenshots, rendered fullscreen inside a single giant <code className="text-accent-light">{'<button>'}</code> so tap-anywhere closes. <strong>Mobile</strong>: portrait <code className="text-accent-light">public/dummy-ad.png</code>. <strong>Desktop</strong>: landscape <code className="text-accent-light">public/dummy-ad-landscape.png</code>. Production wires the real ad SDK (AppLovin / AdMob / Unity) and replaces this component wholesale.</p>
        <div className="flex flex-wrap gap-m items-start">
          <div className="flex flex-col gap-xxs">
            <span className="text-xxs text-text-xsmall uppercase tracking-[0.4px]">Mobile</span>
            <div
              className="bg-black rounded-card border border-white-10 overflow-hidden relative"
              style={{ width: '180px', aspectRatio: '9/19' }}
            >
              <Image
                src="/dummy-ad.png"
                alt="DummyAd Homescapes portrait placeholder"
                fill
          ...
          <div className="flex flex-col gap-xxs">
            <span className="text-xxs text-text-xsmall uppercase tracking-[0.4px]">Desktop</span>
            <div
              className="bg-black rounded-card border border-white-10 overflow-hidden relative"
              style={{ width: '380px', aspectRatio: '16/9' }}
            >
              <Image
                src="/dummy-ad-landscape.png"
                alt="DummyAd Homescapes landscape placeholder"
                fill
                className="object-contain"
                sizes="380px"
              />
      ...
      <div className="min-w-[440px] flex-1">
        ...
        <div className="flex flex-col gap-s text-xs text-text-body bg-white-05 border border-white-10 rounded-card p-m w-full">
          {[
            ['Entry components — <WatchAdBubble /> mounts INSIDE chat column (inline above ChatBar); <WatchAdSheet /> mounts at PAGE ROOT (modal popup, fullscreen scrim)', 'Two separate mount points because modal vs inline have different mount-point requirements: the bubble has to live inside the input-area slot (inherits chat column width); the sheet has to live at page root so its `fixed inset-0` scrim covers the full viewport (Header + Sidebar included). page.tsx renders both, each conditional on its flowMode (`ad-bubble` → bubble open, `ad-sheet` → sheet open). No unified dispatcher.'],
            ...
            ['Sheet anatomy — BottomSheet (mobile) + CenterPopup (desktop) — both mount; each primitive hides itself on the wrong viewport via its own CSS', 'Parallel-mount pattern matches BuyCreditsSheet. Shared <WatchAdBody /> renders inside both wrappers so anatomy is identical. CenterPopup maxWidth=420px on desktop. Z-70 on both. NO secondary link — hard gate, one forward path.'],
            ['Bubble anatomy — inline bubble with eyebrow system-label + title + embedded "Watch ad" button. NO AVATAR. Mobile + desktop, same surface.', 'Mirrors AI bubble chrome exactly (`bg-chat-ai-bubble`, asymmetric rounded corners — flat bottom-left tail, `max-w-chat-bubble`). WSUP\'s ChatMessages.AIBubble does NOT render an avatar (character is set at chat-screen level via ChatHeader); the bubble inherits that convention. System-voice signal comes from the "QUICK AD BREAK" eyebrow label + the embedded Watch ad button. Per the codified taste rule from S23 (character voice vs system voice), the eyebrow label is sufficient differentiation without breaking the chat-bubble convention.'],
            ...
            ['DummyAd — fullscreen placeholder ad (mobile + desktop) that takes over after "Watch ad" is tapped', 'Replaces the production rewarded-video ad SDK (AppLovin / AdMob / Unity Ads) in the demo. Two orientations, one per viewport: mobile = `public/dummy-ad.png` (portrait Homescapes "How to Loot?"); desktop = `public/dummy-ad-landscape.png` (landscape variant). Both render fullscreen inside a single giant `<button>` — TAP ANYWHERE to close. No countdown, no separate close X — the entire surface IS the dismiss affordance, matching the common ad pattern where the ad surface itself is the click-through to the App Store. Production wires the real SDK and replaces DummyAd wholesale; the rewarded-callback path triggers the same `completeWatchAd` handler that DummyAd\'s `onComplete` does today.'],
