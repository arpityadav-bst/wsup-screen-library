    ...
    <Section id="Model Deprecated Sheet" title="Model Deprecated Sheet" onVisible={onSectionVisible}>
      ...
      <div className="min-w-[440px] flex-1">
        ...
        <div className="flex flex-col gap-s text-xs text-text-body bg-white-05 border border-white-10 rounded-card p-m w-full">
          {[
            ['Container — BottomSheet primitive (mobile-only). Desktop = no render (md:hidden).', 'Reuse: same primitive as CreditServicePopup / ChatStyleSheet / StreakClaimPopup. NO CenterPopup pair — this popup is mobile-only by design (Flutter app context). Web /chat fires it via dev panel for preview; desktop sees nothing when state is active.'],
            ...
            ['Eyebrow label — text-xxs uppercase tracking-[0.4px] text-text-dim font-medium, centered', '"CHAT MODEL UPDATE" eyebrow sits above the title to give naive users category context BEFORE they read the body. Reuses the WatchAdBubble "QUICK AD BREAK" eyebrow pattern — established convention for "this is a system-voice category signal." By the time the user reads "Llama 3" in the body, they already know it\'s about their chat model. Solves the "what is Llama 3?" confusion for users who never opened LLM settings.'],
            ...
            ['Secondary text-link — "Pick a different model" → opens ChatStyleSheet', 'Uses .link utility class. Escape hatch for users who want to compare or switch. Closes this popup + opens the unified picker (ChatStyleSheet — same picker triggered by the ChatBar LLM pill and chat-start). Handled by onPickModel prop in page.tsx → setChatState(\'chat-style-popup\').'],
            ...
            ['Z-index — sheet at 70 (matches CreditServicePopup / ChatStyleSheet / StreakClaimPopup)', 'Mutually exclusive with other state-popups (chat state machine is single-active).'],
          ].map(([cls, label]) => (
