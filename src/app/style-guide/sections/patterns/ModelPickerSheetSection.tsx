      ...
      <div className="min-w-[440px] flex-1">
        ...
        <div className="flex flex-col gap-s text-xs text-text-body bg-white-05 border border-white-10 rounded-card p-m w-full">
          {[
            ...
            ['Chip — shared from chat/ModelPickerInternals (NOT the global Badge primitive)', 'bg-white-10 + rounded-[4px] (compact slight-roundness) + px-xxs py-xxxs + gap-xxxs + text-xxs font-medium text-text-small + whitespace-nowrap. Now consumed by both ModelPickerSheet AND ChatStyleSheet (S31 — Gate 3 promotion when ChatStyleSheet adopted the same row anatomy). Local-not-global because Badge\'s pill style is still right for other surfaces (CreditSidebar)'],
            ...
            ['Mock data — lib/models.ts MODELS array (7 entries)', 'Curated lineup post-S31: 4 primary (Llama 3, Qwen Plus Character, DeepSeek V3, Claude Opus 4.6) + 3 other (Mistral Nemo, MiniMax M2 Her, DeepSeek V4 Pro). Each model: id, name, personality, cost (null=Free | number=credits/msg), complexity (1-4 drives signal bars), tier (\'primary\' | \'other\'), description'],
          ].map(([cls, label]) => (
