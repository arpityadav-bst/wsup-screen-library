      ...
      <div className="w-full flex flex-col gap-xl">
        ...
        <div>
          ...
          <div className={DECK_SHELL}>
            <OnboardingDeckStep index={0} canRewind={false} onSkipCard={() => {}} onLikeCard={() => {}} onRewindCard={() => {}} onSkipFlow={() => {}} onRestartDeck={() => {}} />
          </div>
        </div>

        <div>
          <SubLabel>Stage 2 — Deck (card 3, regular — rewind button visible)</SubLabel>
          <div className={DECK_SHELL}>
            <OnboardingDeckStep index={2} canRewind={true} onSkipCard={() => {}} onLikeCard={() => {}} onRewindCard={() => {}} onSkipFlow={() => {}} onRestartDeck={() => {}} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className={DECK_SHELL}>
            <OnboardingDeckStep index={ONBOARDING_DECK.length} canRewind={false} onSkipCard={() => {}} onLikeCard={() => {}} onRewindCard={() => {}} onSkipFlow={() => {}} onRestartDeck={() => {}} />
          </div>
        </div>

        <div>
          <SubLabel>End-state — Show me more / Go to Explore</SubLabel>
          <div className="w-full max-w-popup-medium h-[480px] bg-page-bg rounded-popup border border-white-10 flex flex-col p-l">
            <OnboardingDeckEmptyState onShowMore={() => {}} onGoToExplore={() => {}} />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-xs max-w-[720px]">
          ...
          <div className="rounded-card border border-white-10 p-m text-xs leading-relaxed text-text-body">
            <ul className="list-disc list-inside space-y-xxs">
              <li><span className="text-text-title font-medium">Wrapper</span> — Mobile: <code className="text-accent-light">fixed inset-0 bg-page-bg</code> (full-viewport takeover). Desktop: centered ~480px popup over <code className="text-accent-light">bg-black-70</code> scrim. <em>Height is stage-aware</em>: preferences = <code className="text-accent-light">auto</code> (content-driven), deck = <code className="text-accent-light">min(880px, 94vh)</code> (card stack needs vertical room). <em>Not</em> CenterPopup — custom scrim avoids overflow-hidden clipping.</li>
              <li><span className="text-text-title font-medium">Header</span> — codified <code className="text-accent-light">/logo.png</code> (104×24 composite — same asset shared/Header uses) on left, &ldquo;× Skip&rdquo; pill (secondary chrome) on right. Shared between Stage 1 + Stage 2 + end-state. Skip is the universal escape to /explore during the active deck; the end-state surfaces &ldquo;Go to Explore&rdquo; as a primary in-flow CTA because at end-of-deck exit IS the natural next action (not a duplicate affordance — different scope).</li>
              ...
              <li><span className="text-text-title font-medium">Stage 2 (Deck)</span> — Title + helper line; DeckProgressBars (segmented, one per card); deck stack of up to 3 cards (top + 2 peeks behind); DeckActionButtons (wider <code className="text-accent-light">× Pass</code> / <code className="text-accent-light">♥ Like</code> pills, flex-1 each). After the user&apos;s first pass, a tertiary icon-only <code className="text-accent-light">Rewind</code> button (52×52 circular, neutral chrome — no status color) appears to the LEFT of Pass; intentionally not flex-1 so Pass/Like keep visual primacy.</li>
              ...
              <li><span className="text-text-title font-medium">End-state</span> — Renders when <code className="text-accent-light">index &gt;= deck.length</code> (every card skipped without a like). Stacked-cards illustration + heading + two CTAs: <code className="text-accent-light">Show me more</code> (primary — load next batch in production) and <code className="text-accent-light">Go to Explore</code> (secondary — exit to /explore; same destination as the header Skip pill). Subtitle uses <code className="text-accent-light">text-balance</code> (CSS <code className="text-accent-light">text-wrap: balance</code>) so it sits single-line on desktop and breaks at the comma on mobile — no orphan words. See Typography → Line-break hygiene for the codified rule.</li>
              <li><span className="text-text-title font-medium">Tag chips</span> — Both Stage 1 deck preview and DeckCard tags consume the shared <code className="text-accent-light">{'<CharacterTagChip>'}</code> primitive (`components/shared/CharacterTagChip.tsx`) — same chrome as CharacterCard on /explore and ChatRightSidebar on /chat. Single source of truth for character tag styling.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-xs max-w-[720px]">
          ...
          <div className="rounded-card border border-white-10 p-m text-xs leading-relaxed text-text-body">
            <ul className="list-disc list-inside space-y-xxs">
              ...
              <li>Deck position is React state only — re-entering &ldquo;Onboarding&rdquo; via the dev panel starts at preferences again. <code className="text-accent-light">canRewind</code> derives from <code className="text-accent-light">deckIndex &gt; 0</code> (rewind button shows once the user has passed at least once). End-state CTAs: &ldquo;Show me more&rdquo; resets the index to 0 (production: load next batch); &ldquo;Go to Explore&rdquo; calls <code className="text-accent-light">onSkipFlow</code> (same as the header Skip pill — closes the onboarding overlay back to /explore).</li>
            </ul>
          </div>
        </div>
