  ...
  return (
    <Section id="Chat Style Sheet" title="Chat Style Sheet" onVisible={onSectionVisible}>
      <div className="min-w-[440px] flex-1">
        <SubLabel>The UNIFIED model picker for /chat. Triggered from THREE entry points: (1) starting a new chat (returning-user flow, mocked via R-key dev state &quot;Model selection&quot;), (2) tapping the LLM pill in ChatBar, (3) tapping &quot;Pick a different model&quot; on ModelDeprecatedSheet. Multi-step flow: primary models → other models → app-handoff. Row anatomy uses <code>ModelRow</code> from <code>ModelPickerInternals</code> (the picker-row primitive that survived the S33-close consolidation when ModelPickerSheet was retired).</SubLabel>
...
{primaryModels.map((m) => <MockRow key={m.id} model={m} selected={m.id === 'deepseek-v3'} />)}
      ...
      <div className="min-w-[440px] flex-1">
        ...
        <div className="flex flex-col gap-s text-xs text-text-body bg-white-05 border border-white-10 rounded-card p-m w-full">
          {[
            ['Container — BottomSheet (mobile) + CenterPopup (desktop) paired pattern, maxWidth 420px', 'Reuse: same primitives as BuyCreditsSheet / LoginSheet / StreakClaimPopup. Single PickerBody renders identical content into both sheets'],
            ['Surface — bg-profile-sheet-bg, rounded-popup (24px), border-white-10, shadow-popup', 'Codified centered-popup chrome (matches StreakClaimPopup, MemoryLimitPopup). No new surface tokens'],
            ...
            ['Two-step flow — useState<\'primary\' | \'other\'>; on every open, step is set to match the pre-selected model\'s tier (\'other\' if selectedId is in other tier, else \'primary\')', 'Modeled on BuyCreditsSheet\'s multi-step pattern. Single sheet, internal step state, no router. Init-from-selectedId: when triggered mid-chat via the ChatBar LLM pill, the user\'s current model is pre-selected so they don\'t re-pick. New-chat trigger (selectedId undefined) starts with draft=null + step=primary.'],
            ['selectedId prop (optional) — when provided, draft initializes to this model on open + Continue CTA is enabled immediately', 'Mid-chat trigger (ChatBar LLM pill) passes selectedId={currentModel.id} so the picker opens with the user\'s active model pre-highlighted. New-chat trigger (returning-user flow) omits selectedId so the picker requires explicit selection before Continue is enabled. Same picker, two init modes — no chrome difference between them.'],
            ['Step 1 (primary) — 4 model rows + Other disclosure + Continue chat CTA', '4 primary-tier models (Llama 3, Qwen Plus, DeepSeek V3, Claude Opus 4.6) drawn from MODELS.filter(m => m.tier === \'primary\'). Cross-app promo is now data-driven via per-row appOnly chips (Llama 3 has GradientChip "Available only in mobile app") + dynamic "Continue in app →" CTA — the standalone "Free models on mobile app" banner was removed in S31 since the row-level signal made it redundant. Other disclosure is a row-shaped button with right chevron. CTA = primary Button fullWidth with disabled={draft === null} — disabled until user taps a row, enabled after'],
            ...
            ['ModelRow — shared from chat/ModelPickerInternals.tsx; chip visibility flagged via showSignal / showPersonality props', 'Picker-row primitive. ChatStyleSheet passes showSignal + showPersonality based on the step (full chips on advanced/other; pared-down on the simpler primary tier). The component stays identical; the surface decides what metadata to surface for its choice context.'],
            ['Available-only-in-app chip — auto-rendered by ModelRow when model.appOnly is true', 'GradientChip primitive (also exported from ModelPickerInternals): same shape as Chip but uses WSUP brand gradient (from-gradient-purple/10 to-gradient-blue/10 bg + matching text gradient via bg-clip-text text-transparent). S31 tagged Llama 3 with appOnly: true since it\'s mobile-app-tier. ChatStyleSheet auto-renders the chip — data-driven, not surface-specific.'],
            ['Selected state — ring-2 ring-accent + bg-accent/10 + CheckBadge absolute top-1/2 right-m', 'CheckBadge = 28px green circle with checkmark, distinct from Checkbox primitive (square 16px togglable role).'],
            ['Selection commit semantics — draft selection inside sheet (initially null); commits ONLY on Continue chat tap, NOT on row tap', 'Explicit-commit semantics. The user picks a row (draft), then taps Continue chat to commit. Initial null draft enforces the disabled-CTA-until-selection behavior. Applies to both new-chat and mid-chat entry points after the S33-close consolidation.'],
            ...
            ['Z-index — 70 (matches the popup family)', 'Above SafetyBanner overlay (20), above MemoryLimitOverlay anchored mode (30). Same z as CreditServicePopup, StreakClaimPopup, ModelDeprecatedSheet.'],
          ].map(([cls, label]) => (
