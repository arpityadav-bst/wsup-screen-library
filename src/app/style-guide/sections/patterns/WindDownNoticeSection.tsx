      ...
      <div className="min-w-[540px] flex-1">
        ...
        <div className="flex flex-col gap-s text-xs text-text-body bg-white-05 border border-white-10 rounded-card p-m w-full">
          {[
            ...
            ['AppCard primitive — Gate 3 extracted at 2 consumers', 'src/components/shared/AppCard.tsx. 48px logo + name in 2-column flex row, bg-white-05 border-white-10, full-width. Consumed by WindDownPopup + WindDownDetailsPopup for the "Other apps to try" section. Logos at public/external/polybuzz.png and talkie.png.'],
            ['Details popup — no CTAs, email contact at bottom', 'Refund + Download request paths funnel to email (refund@wsup.ai / data@wsup.ai placeholders) instead of in-app action buttons. Email lines flow naturally after the personal note (not sticky, no border-top) — read as part of the message close, not an action footer.'],
            ...
            ['Color exemption — bg-status-warning saturated chrome on hero icon', 'Product-state announcement (wind-down) is exempt from the codified "Warnings are informative, not alarming, never a fully saturated surface" rule (taste.md line 367 — S35 scope amendment). The amendment scopes that rule to user-state warnings; product-state announcements where the user cannot change the state are permitted to use saturated alarm color.'],
            ...
            ['Popup dismissal — localStorage, permanent', 'localStorage key wsup:winddown-popup-seen. Once acknowledged, never shows again. Single-exposure model since strip was removed S35 close.'],
            ...
            ['LowCreditsBanner + StreakClaimPopup removed from /explore', 'Acquisition surfaces ("Add credits", "Claim your free credits") contradict wind-down messaging — hidden for the wind-down phase. Component files + style guide sections retained for reference.'],
          ].map(([cls, label]) => (
